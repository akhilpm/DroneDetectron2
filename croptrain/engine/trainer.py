...
class BaselineTrainer(DefaultTrainer):
    ...
    def build_hooks(self):
        ...
        def test_and_save_results():
            if cfg.CROPTRAIN.USE_CROPS:
                self._last_eval_results = self.test_crop(self.cfg, self.model)
            else:
    ...
    @classmethod
    def test_crop(cls, cfg, model, evaluators=None):
        logger = logging.getLogger(__name__)
...
def inference_on_dataset(model, data_loader, evaluator, cfg):
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            new_cluster_boxes = merge_cluster_boxes(cluster_boxes)
            outputs[0]["instances"] = outputs[0]["instances"][~cluster_class_indices]
            cluster_dicts = get_dict_from_crops(new_cluster_boxes, inputs[0], cfg.CROPTRAIN.CROPSIZE)
...
def merge_cluster_boxes(cluster_boxes):
    overlaps = pairwise_iou(cluster_boxes, cluster_boxes)
    connectivity = (overlaps > 0.3)
    ...
    while len(connectivity)>0:
        ...
        cluster_members = cluster_boxes.tensor[cluster_components]
